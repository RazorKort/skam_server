...
async def get_friends(user: GetFriends):
    ...
    if not rows:
        return {'status':'ok', 'details':'lonely'}
    else:
...
async def msgs(user: LoadMessages):
    ...
    async with app.state.pool.acquire() as conn:
        ...
        if not rows:
            return {'status':'ok', 'message':'сообщений нет'}
        else: 
...
async def search(user: Search):
    query = 'SELECT id, nickname FROM users WHERE nickname LIKE $1'
    async with app.state.pool.acquire() as conn:
